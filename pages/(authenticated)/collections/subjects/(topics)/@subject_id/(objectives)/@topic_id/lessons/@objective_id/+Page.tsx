...
export default function () {
  ...
  useEffect(() => {
    new WithAuth(
      postgrest.from("lesson_view").select().eq(
        "objective_id",
        objective_id,
      ),
    ).unwrap().then(
  ...
  return (
    <div ref={containerRef}>
        ...
        <Button
          ...
          onClick={() =>
            mountModal(
              ...
              {
                title:
                  `Create Lesson - ${context.config.collection?.objective_title}`,
              },
            )}
        />
      </div>
      <Table<Lesson>
        ...
        rows={lessons.sort((a, b) => sortTag(a.tag, b.tag)).map((
          v,
          idx,
        ) => ({
          value: { ...v, idx: idx + 1 },
            ...
            switch (idx) {
              ...
              case "idx":
                return (
                  <div className="flex gap-2 group w-10 items-center">
                    <span className="w-5">
                      {(lesson as Record<string, any>)[idx]}
                    </span>
                    <ExpandIcon
              ...
              default:
                return (
                  <span
                    dangerouslySetInnerHTML={{
                      __html: (lesson as Record<string, any>)[idx],
                    }}
                  />
                );
            }
