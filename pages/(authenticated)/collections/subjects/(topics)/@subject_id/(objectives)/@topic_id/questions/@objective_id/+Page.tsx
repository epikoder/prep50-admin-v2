...
export default function () {
  ...
  useEffect(() => {
    new WithAuth(
      postgrest.from("question_view").select().eq(
        "objective_id",
        objective_id,
      ),
    ).unwrap().then(
  ...
  return (
    <Fragment>
                ...
                title: (
                  <span
                    dangerouslySetInnerHTML={{
                      __html:
                        `Create Question - ${context.config.collection?.objective_title}`,
                    }}
                  />
                ),
              },
      ...
      <Table<Question>
        ...
        rows={questions.sort((a, b) => sortTag(a.tag, b.tag)).map((
          v,
          idx,
        ) => ({
          value: { ...v, idx: idx + 1 },
          render: (question, idx) => {
            switch (idx) {
              ...
              case "short_answer":
                return (
                  <span className="whitespace-nowrap">
                    {questionOptionString(question.short_answer)}
                  </span>
                );
              case "idx":
                return (
                  <div className="flex gap-2 group w-10 items-center">
                    <span className="w-5">
                      {(question as Record<string, any>)[idx]}
                    </span>
                    <ExpandIcon
              ...
              default:
                return (
                  <span
                    ...
                    dangerouslySetInnerHTML={{
                      __html: (question as Record<string, any>)[idx],
                    }}
                  />
                );
            }
...
const FormComponent = (
  ...
  const onSubmit = async () => {
    ...
    if (!formData) {
      ...
      await new WithAuth(
        postgrest.from("objective_lessons").update({
          objective_id: objectiveId,
