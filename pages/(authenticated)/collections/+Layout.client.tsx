...
function Content({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const subject_id = context.routeParams.subject_id;
    {
      if (subject_id && subject_id != "0") {
        new WithAuth(
          postgrest.from("subjects").select("name").eq(
            "id",
            subject_id,
          ).single(),
    ...
    const topic_id = context.routeParams.topic_id;
    {
      if (topic_id && topic_id != "0") {
        new WithAuth(
          postgrest.from("topics").select("title").eq(
            "id",
            topic_id,
          ).single(),
        ).unwrap().then(({ data, error }) => {
    ...
    const objective_id = context.routeParams.objective_id;
    {
      if (objective_id && objective_id != "0") {
        new WithAuth(
          postgrest.from("objectives").select("title").eq(
            "id",
            objective_id,
          ).single(),
        ).unwrap().then(({ data, error }) => {
          ...
          links[2] = {
            ...
            name: (data as Topic).title,
            href:
              `/collections/subjects/${subject_id}/${topic_id}/${objective_id}`,
          };
          context.config.collection = {
            ...context.config.collection,
  ...
  useEffect(() => {
    let path = context.urlPathname.slice(1).split("/").slice(0,2).join(" ")
      .replace(
        "collections",
        "",
      ).trim();
